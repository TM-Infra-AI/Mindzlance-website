 ...
 * Text Domain: woodiscuz-woocommerce-comments
 * Domain Path: /languages/
 * WC tested up to: 5.1
 */

define('WOODISCUZDS', DIRECTORY_SEPARATOR);
define('WOODISCUZ_DIR_PATH', dirname(__FILE__));
define('WOODISCUZ_DIR_NAME', basename(WOODISCUZ_DIR_PATH));

include_once 'options/wpc-options.php';
include_once 'options/wpc-options-serialize.php';
include_once 'includes/wpc-helper.php';
...
include_once 'widgets/widget-woocommerce-reviews.php';
include_once 'widgets/widget-woodiscuz-comments.php';
...
class WPC_Core {
    ...
    function __construct() {
        ...
        $this->woocommerceVersion = WPC_Helper::wpcWoocommerceVersion();
        ...
        if (version_compare($this->woocommerceVersion, '3.1.2', '<')) {
            add_filter('woocommerce_product_review_count', array(&$this, 'woodiscuz_get_reviews_count'), 1234);
        } else {
            add_filter('woocommerce_product_get_review_count', array(&$this, 'woodiscuz_get_reviews_count'), 1234);
        }
    ...
    public function getCommentsArgs($args, $obj) {
        ...
        if (isset($post) && $obj->query_vars['type'] != 'woodiscuz' && $post->post_type == 'product') {
            $args['where'] .= " AND `comment_type` = 'review' ";
        }
        return $args;
    }
    ...
    public function woodiscuz_get_reviews_count() {
        global $post;
        $this->reviews_count = $this->wpc_db_helper->get_reviews_count('review', $post->ID);
        return $this->reviews_count;
    }

    public function load_woodiscuz_text_domain() {
        load_plugin_textdomain('woodiscuz-woocommerce-comments', false, WOODISCUZ_DIR_NAME . '/languages/');
    }

    public function woop_disscus_requirements() {
        if (!is_plugin_active('woocommerce/woocommerce.php')) {
            echo "<div class='error'><p>" . __('WooDiscuz requires Woocommerce to be installed!', 'woodiscuz-woocommerce-comments') . "</p></div>";
        }
    ...
    public function wpc_plugin_new_version() {
        ...
        if (version_compare($wpc_plugin_data['Version'], $wpc_version, '>')) {
            ...
            if (version_compare($wpc_version, '2.2.0', '<')) {
                $this->wpc_db_helper->update_comments_types_2_2_0();
            }
    ...
    public function add_comment_tab($tabs) {
        ...
        $priority = ($pr = absint($this->wpc_options_serialized->wpc_comment_tab_priority)) ? $pr : 1000;
    ...
    public function add_comment_type($args) {
        ...
        $args['woodiscuz'] = __('WooDiscuz', 'woodiscuz-woocommerce-comments');
        $args['review'] = __('Woocomerce Review', 'woodiscuz-woocommerce-comments');
        return $args;
    }
    ...
    public function add_plugin_options_page() {
        if (function_exists('add_options_page')) {
            add_menu_page('WooDiscuz', 'WooDiscuz', 'manage_options', 'woodiscuz_options_page', array(&$this->wpc_options, 'main_options_form'), plugins_url(WOODISCUZ_DIR_NAME . '/files/img/plugin-icon/plugin-icon-20.png'), 1245);
    ...
    public function front_end_styles_scripts() {
        ...
        if (preg_match('/MSIE/i', $u_agent)) {
            wp_enqueue_script('woodiscuz-html5-js', plugins_url(WOODISCUZ_DIR_NAME . '/files/third-party/tooltipster/js/html5.js'), array('jquery'), '1.2', false);

            wp_register_style('woodiscuz-modal-css-ie', plugins_url(WOODISCUZ_DIR_NAME . '/files/third-party/modal-box/modal-box-ie.css'), null, get_option($this->woodiscuz_version));
            wp_enqueue_style('woodiscuz-modal-css-ie');
        }

        wp_register_style('woodiscuz-modal-box-css', plugins_url(WOODISCUZ_DIR_NAME . '/files/third-party/modal-box/modal-box.css'), null, get_option($this->woodiscuz_version));
        wp_enqueue_style('woodiscuz-modal-box-css');

        wp_enqueue_script('woodiscuz-validator-js', plugins_url(WOODISCUZ_DIR_NAME . '/files/js/validator.js'), array('jquery'), '1.0.0', false);

        wp_register_style('woodiscuz-validator-style', plugins_url(WOODISCUZ_DIR_NAME . '/files/css/fv.css'), null, get_option($this->woodiscuz_version));
        wp_enqueue_style('woodiscuz-validator-style');

        wp_enqueue_script('woodiscuz-ajax-js', plugins_url(WOODISCUZ_DIR_NAME . '/files/js/wpc-ajax.js'), array('jquery'), get_option($this->woodiscuz_version), false);
        wp_localize_script('woodiscuz-ajax-js', 'wpc_ajax_obj', array('url' => admin_url('admin-ajax.php')));

        wp_enqueue_script('woodiscuz-cookie-js', plugins_url(WOODISCUZ_DIR_NAME . '/files/js/jquery.cookie.js'), array('jquery'), '1.4.1', false);

        wp_register_style('woodiscuz-tooltipster-style', plugins_url(WOODISCUZ_DIR_NAME . '/files/third-party/tooltipster/css/tooltipster.css'), null, get_option($this->woodiscuz_version));
        wp_enqueue_style('woodiscuz-tooltipster-style');

        wp_enqueue_script('woodiscuz-tooltipster-js', plugins_url(WOODISCUZ_DIR_NAME . '/files/third-party/tooltipster/js/jquery.tooltipster.min.js'), array('jquery'), '1.2', false);
        wp_enqueue_script('woodiscuz-footer-js', plugins_url(WOODISCUZ_DIR_NAME . '/files/js/wpc-footer-script.js'), array('jquery'), '1.2', true);

        wp_enqueue_script('woodiscuz-autogrowtextarea-js', plugins_url(WOODISCUZ_DIR_NAME . '/files/js/jquery.autogrowtextarea.min.js'), array('jquery'), '3.0', false);

        wp_register_style('woodiscuz-frontend-css', plugins_url(WOODISCUZ_DIR_NAME . '/files/css/woodiscuz-frontend.css'), null, get_option($this->woodiscuz_version));
    ...
    public function admin_page_styles_scripts() {
        ...
        wp_register_style('woodiscuz-colorpicker-css', plugins_url(WOODISCUZ_DIR_NAME . '/files/third-party/colorpicker/css/colorpicker.css'), null, get_option($this->woodiscuz_version));
        wp_enqueue_style('woodiscuz-colorpicker-css');

        wp_register_script('woodiscuz-colorpicker-js', plugins_url(WOODISCUZ_DIR_NAME . '/files/third-party/colorpicker/js/colorpicker.js'), array('jquery'), '2.0.0.9', false);
        wp_enqueue_script('woodiscuz-colorpicker-js');

        wp_register_style('woodiscuz-options-css', plugins_url(WOODISCUZ_DIR_NAME . '/files/css/options-css.css'), null, get_option($this->woodiscuz_version));
        wp_enqueue_style('woodiscuz-options-css');

        wp_register_script('woodiscuz-scripts-js', plugins_url(WOODISCUZ_DIR_NAME . '/files/js/wpc-scripts.js'), array('jquery'));
        wp_enqueue_script('woodiscuz-scripts-js');


        wp_register_style('woodiscuz-easy-responsive-tabs-css', plugins_url(WOODISCUZ_DIR_NAME . '/files/third-party/easy-responsive-tabs/css/easy-responsive-tabs.css'), true);
        wp_enqueue_style('woodiscuz-easy-responsive-tabs-css');

        wp_register_script('woodiscuz-easy-responsive-tabs-js', plugins_url(WOODISCUZ_DIR_NAME . '/files/third-party/easy-responsive-tabs/js/easy-responsive-tabs.js'), array('jquery'), '1.0.0', true);
    ...
    public function comment_submit_via_ajax() {
        ...
        if (!$this->wpc_options_serialized->wpc_captcha_show_hide) {
            if (!is_user_logged_in()) {
                ...
                if (md5(strtolower($captcha)) !== $sess_captcha) {
                    $messageArray['code'] = -1;
                    $messageArray['message'] = $this->wpc_options_serialized->wpc_phrases['wpc_invalid_captcha'];
        ...
        if ($user && $user->ID) {
            ...
            if ($this->wpc_options_serialized->wpc_is_name_field_required) {
                ...
                $name = !(filter_input(INPUT_POST, 'name')) ? __('anonymous', 'woodiscuz-woocommerce-comments') : filter_input(INPUT_POST, 'name');
            }
        ...
        if ($name && filter_var($email, FILTER_VALIDATE_EMAIL) && $comment && filter_var($comment_post_ID)) {
            ...
            if ($held_moderate) {
                $messageArray['code'] = -2;
                $messageArray['message'] = $this->wpc_options_serialized->wpc_phrases['wpc_held_for_moderate'];
            } else {
                $messageArray['code'] = 1;
                $messageArray['message'] = $this->comment_tpl_builder->get_comment_template($new_comment, null, $comment_depth);
                $messageArray['is_in_same_container'] = $is_in_same_container;
            }
            $messageArray['wpc_new_comment_id'] = $new_comment_id;
            ...
            $messageArray['code'] = -1;
            $messageArray['wpc_new_comment_id'] = -1;
            $messageArray['message'] = $this->wpc_options_serialized->wpc_phrases['wpc_invalid_field'];
        }
        wp_die(json_encode($messageArray));
    }
        ...
        if ($wpc_comment_id) {
            ...
            if ($comment->comment_ID) {
                ...
                if ($this->wpc_options_serialized->woodiscuz_redirect_page && $wpc_user_comment_count == 1) {
                    $messageArray['code'] = 1;
                    $messageArray['redirect_to'] = get_permalink($this->wpc_options_serialized->woodiscuz_redirect_page);
                }
            }
        } else {
            $messageArray['code'] = -1;
...
// Add settings link on plugin page
    ...
    public function wpc_get_editable_comment_content() {
        ...
        if ($comment_ID) {
            ...
            if (isset($current_user) && $comment->user_id == $current_user->ID && $this->wpc_helper->is_comment_editable($comment)) {
                ...
                $messageArray['code'] = -1;
                $messageArray['phrase_message'] = $this->wpc_options_serialized->wpc_phrases['wpc_comment_edit_not_possible'];
            }
        } else {
            $messageArray['code'] = -1;
            $messageArray['phrase_message'] = $this->wpc_options_serialized->wpc_phrases['wpc_comment_edit_not_possible'];
        ...
        if ($trimmed_comment_content && isset($current_user) && $comment->user_id == $current_user->ID) {
            if ($trimmed_comment_content != $comment->comment_content) {
                ...
                if (wp_update_comment($commentarr)) {
                    $messageArray['code'] = 1;
                    $messageArray['message'] = $this->wpc_helper->make_clickable($comment_content);
                } else {
                    $messageArray['code'] = -1;
                    $messageArray['phrase_message'] = $this->wpc_options_serialized->wpc_phrases['wpc_comment_not_updated'];
                }
            } else {
                $messageArray['code'] = -2;
                $messageArray['phrase_message'] = $this->wpc_options_serialized->wpc_phrases['wpc_comment_not_edited'];
            }
        } else {
            $messageArray['code'] = -1;
            $messageArray['phrase_message'] = $this->wpc_options_serialized->wpc_phrases['wpc_comment_edit_not_possible'];
    ...
    public function wpc_confirm_email_sender($subscribe_id, $email, $post_id, $new_comment_id) {
        ...
        $subject = isset($this->wpc_options_serialized->wpc_phrases['wpc_confirm_email_subject']) ? $this->wpc_options_serialized->wpc_phrases['wpc_confirm_email_subject'] : __('Subscribe Confirmation', 'woodiscuz-woocommerce-comments');
    ...
    public function wpc_notify_on_new_reply($parent_comment_id, $new_comment_id, $email) {
        ...
        $subject = ($this->wpc_options_serialized->wpc_phrases['wpc_new_reply_email_subject']) ? $this->wpc_options_serialized->wpc_phrases['wpc_new_reply_email_subject'] : __('New Reply', 'woodiscuz-woocommerce-comments');
        $message = ($this->wpc_options_serialized->wpc_phrases['wpc_new_reply_email_message']) ? $this->wpc_options_serialized->wpc_phrases['wpc_new_reply_email_message'] : __('New reply on the discussion section you\'ve been interested in', 'woodiscuz-woocommerce-comments');
