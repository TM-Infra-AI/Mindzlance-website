...
class WPC_Comment_Template_Builder {
    ...
    public function get_comment_template($comment, $args, $depth) {
        ...
        $wpc_comment_wrapper_class .= ' ' . $this->get_author_class_by_user($user);
        ...
        if (!$this->wpc_options_serialized->wpc_share_buttons_show_hide) {
            ...
            $output .= '<a target="_blank" href="http://www.facebook.com/sharer.php?u=' . $wpc_post_permalink . '" title="' . $this->wpc_options_serialized->wpc_phrases['wpc_share_facebook'] . '"><img src="' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/fb-18x18.png') . '" onmouseover="this.src=\'' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/fb-18x18-orig.png') . '\'" onmouseout="this.src=\'' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/fb-18x18.png') . '\'"/></a>&nbsp;&nbsp;';
            $output .= '<a target="_blank" href="https://twitter.com/home?status=' . $twitt_content . '" title="' . $this->wpc_options_serialized->wpc_phrases['wpc_share_twitter'] . '"><img src="' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/twitter-18x18.png') . '" onmouseover="this.src=\'' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/twitter-18x18-orig.png') . '\'" onmouseout="this.src=\'' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/twitter-18x18.png') . '\'"/></a>&nbsp;&nbsp;';
            $output .= '<a target="_blank" href="https://plus.google.com/share?url=' . $wpc_post_permalink . '" title="' . $this->wpc_options_serialized->wpc_phrases['wpc_share_google'] . '"><img src="' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/google-18x18.png') . '" onmouseover="this.src=\'' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/google-18x18-orig.png') . '\'" onmouseout="this.src=\'' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/google-18x18.png') . '\'"/></a>&nbsp;&nbsp;';
            $output .= '<a target="_blank" href="http://vk.com/share.php?url=' . $wpc_post_permalink . '" title="' . $this->wpc_options_serialized->wpc_phrases['wpc_share_vk'] . '"><img src="' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/vk-18x18.png') . '" onmouseover="this.src=\'' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/vk-18x18-orig.png') . '\'" onmouseout="this.src=\'' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/vk-18x18.png') . '\'"/></a>&nbsp;&nbsp;';
            $output .= '<a target="_blank" href="http://www.odnoklassniki.ru/dk?st.cmd=addShare&st.s=1&st._surl=' . $wpc_post_permalink . '" title="' . $this->wpc_options_serialized->wpc_phrases['wpc_share_ok'] . '"><img src="' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/ok-18x18.png') . '" onmouseover="this.src=\'' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/ok-18x18-orig.png') . '\'" onmouseout="this.src=\'' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/social-icons/ok-18x18.png') . '\'"/></a>&nbsp;&nbsp;';
            $output .= '</span>';
        }
        ...
        if ($this->is_form_visible()) {
            ...
            $output .= '<div class="wpc-form-submit">';
			 ...
			 if (!$this->wpc_options_serialized->wpc_captcha_show_hide) {
                if (!is_user_logged_in()) {
                    $output .= '<div class="wpc-field-captcha woodiscuz-item"><input id="wpc_captcha-' . $unique_id . '" name="wpc_captcha" required="required" value="" type="text" /><span class="wpc-label wpc-captcha-label"><img src="' . plugins_url(WOODISCUZ_DIR_NAME . '/captcha/captcha.php?comm_id=' . $comment->comment_post_ID . '-' . $comment->comment_ID) . '" id="wpc_captcha_img-' . $unique_id . '" /><img src="' . plugins_url(WOODISCUZ_DIR_NAME . '/files/img/refresh-16x16.png') . '" id="wpc_captcha_refresh_img-' . $unique_id . '" class="wpc_captcha_refresh_img" /></span><span class="captcha_msg">' . $this->wpc_options_serialized->wpc_phrases['wpc_captcha_text'] . '</span></div>';
                }
            }
    ...
    /**
     * get comment author class
     */
    public function get_author_class_by_user($user) {
        if ($user) {
            $roles = $user->roles;

            foreach ($roles as $role) {
                if (in_array($role, $this->wpc_options_serialized->wpc_usergroups_for_support_title)) {
                    return 'wpc-role-support';
                }
            }

            if (in_array('customer', $roles)) {
                return 'wpc-role-customer';
            } else {
                return 'wpc-role-member';
            }
        } else {
            return 'wpc-role-guest';
        }
    }
