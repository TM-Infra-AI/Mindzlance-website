    ...
    <div class="dokan-product-attribute-item dokan-clearfix dokan-hide">
        <div class="content-half-part">
			...
			<label class="checkbox-item form-label">
				<input type="checkbox" <?php checked( $attribute['is_visible'], 1 ); ?> onchange="hi(<?php echo $i; ?>);" name="attribute_visibility[<?php echo $i; ?>]" value="1" /> <?php _e( 'Visible on the product page', 'dokan' ); ?>
			</label>
			...
			<?php if ( $attribute['is_taxonomy'] ) : ?>
				<?php if ( 'select' === $attribute_taxonomy->attribute_type ) : ?>
					...
					<div class="dokan-pre-defined-attribute-btn-group">
						...
						<button id="<?php echo $i; ?>" class="dokan-btn dokan-btn-default minus dokan-select-no-attributes"><?php _e( 'Select none', 'dokan' ); ?></button>
...
<script type="text/javascript">
	function hi(a){
	// 	var manage = jQuery(this).is(":checked");
		// alert(a);
	// jQuery(".dokan_attribute_values option:selected").removeAttr("selected");	
	jQuery("#"+a).click();
