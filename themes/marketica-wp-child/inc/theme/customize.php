...
define( 'TOKOPRESS_CUSTOMIZER_VERSION', '0.2.0' );
...
function tokopress_customize_setting_db() {
	$setting_db = defined( 'TOKOPRESS_THEME_SLUG' ) ? TOKOPRESS_THEME_SLUG : '';
	$setting_db = apply_filters( 'tokopress_customize_setting_db', $setting_db );
	if ( !$setting_db ) {
...
function tokopress_customize_get_mod( $field ) {
	...
	if ( ! isset( $field['default'] ) ) {
		global $tokopress_defaults;
		if ( isset( $tokopress_defaults[ $field['setting'] ] ) ) {
			$field['default'] = $tokopress_defaults[ $field['setting'] ];
		}
		else {
			$field['default'] = false;
		}
...
function tokopress_customize_register( $wp_customize ){
	...
	$wp_customize->register_control_type( 'TokoPress_Customize_Heading_Control' );
	$wp_customize->register_control_type( 'TokoPress_Customize_Warning_Control' );
	$wp_customize->register_control_type( 'TokoPress_Customize_Number_Control' );
	$wp_customize->register_control_type( 'TokoPress_Customize_Color_Control' );
	$wp_customize->register_control_type( 'TokoPress_Customize_Slider_Control' );
	$wp_customize->register_control_type( 'TokoPress_Customize_Radio_Buttonset_Control' );
	$wp_customize->register_control_type( 'TokoPress_Customize_Radio_Iconset_Control' );
	$wp_customize->register_control_type( 'TokoPress_Customize_Radio_Image_Control' );
	$wp_customize->register_control_type( 'TokoPress_Customize_Select2_Control' );
	$wp_customize->register_control_type( 'TokoPress_Customize_Select2_Multiple_Control' );
	$wp_customize->register_control_type( 'TokoPress_Customize_GoogleFonts_Control' );
	$wp_customize->register_control_type( 'TokoPress_Customize_Sortable_Control' );
	...
	if ( ! empty( $fields ) ) {
		foreach ( $fields as $field ) {
			...
			elseif ( 'textarea-html' == $field['type'] ) {
				$defaults['sanitize_callback'] = 'tokopress_customize_sanitize_html';
			}
			elseif ( 'textarea-nohtml' == $field['type'] ) {
				$defaults['sanitize_callback'] = 'tokopress_customize_sanitize_nohtml';
			}
			elseif ( 'textarea-unfiltered' == $field['type'] ) {
				$defaults['sanitize_callback'] = 'tokopress_customize_sanitize_unfiltered';
			}
			elseif ( 'email' == $field['type'] ) {
				$defaults['sanitize_callback'] = 'tokopress_customize_sanitize_email';
			}
			elseif ( 'url' == $field['type'] ) {
				$defaults['sanitize_callback'] = 'tokopress_customize_sanitize_url';
			}
			elseif ( 'number' == $field['type'] ) {
				$defaults['sanitize_callback'] = 'tokopress_customize_sanitize_number';
			}
			elseif ( 'checkbox' == $field['type'] ) {
				$defaults['sanitize_callback'] = 'tokopress_customize_sanitize_checkbox';
			}
			elseif ( in_array( $field['type'], array( 'select', 'select2', 'radio', 'radio-buttonset', 'radio-iconset', 'radio-image' ) ) ) {
				$defaults['sanitize_callback'] = 'tokopress_customize_sanitize_select';
			}
			elseif ( in_array( $field['type'], array( 'sortable', 'select2-multiple' ) ) ) {
				$defaults['sanitize_callback'] = 'tokopress_customize_sanitize_select_multiple';
			}
			elseif ( in_array( $field['type'], array( 'font' ) ) ) {
				$defaults['sanitize_callback'] = 'tokopress_customize_sanitize_select_remote';
			}
			elseif ( 'image' == $field['type'] ) {
				$defaults['sanitize_callback'] = 'tokopress_customize_sanitize_image';
			}
			elseif ( 'color' == $field['type'] ) {
				$defaults['sanitize_callback'] = 'tokopress_customize_sanitize_color';
			}
			elseif ( 'dropdown-pages' == $field['type'] ) {
				$defaults['sanitize_callback'] = 'tokopress_customize_sanitize_pages';
			}
			else {
				$defaults['sanitize_callback'] = 'sanitize_text_field';
			}
			...
			global $tokopress_defaults;
			if ( isset( $tokopress_defaults[ $field['setting'] ] ) ) {
				$defaults['default'] = $tokopress_defaults[ $field['setting'] ];
			}
			...
			if ( $field['setting'] && $field['type'] ) {
				...
				elseif ( 'section' == $field['type'] ) {
					...
					elseif ( in_array( $field['type'], array( 'number' ) ) ) {
						$wp_customize->add_control(
							new TokoPress_Customize_Number_Control( 
								$wp_customize,
								$setting,
								array(
									'settings'		=> $setting,
					...
					elseif ( 'radio-iconset' == $field['type'] ) {
						$wp_customize->add_control(
							new TokoPress_Customize_Radio_Iconset_Control( 
								$wp_customize,
								$setting,
								array(
									'settings'		=> $setting,
					...
					elseif ( 'select2-multiple' == $field['type'] ) {
						$wp_customize->add_control(
							new TokoPress_Customize_Select2_Multiple_Control( 
								$wp_customize,
								$setting,
								array(
									'settings'		=> $setting,
					...
					elseif ( 'font' == $field['type'] ) {
						$wp_customize->add_control(
							new TokoPress_Customize_GoogleFonts_Control( 
								$wp_customize,
								$setting,
								array(
									'settings'		=> $setting,
					...
					elseif ( 'sortable' == $field['type'] ) {
						$wp_customize->add_control(
							new TokoPress_Customize_Sortable_Control( 
								$wp_customize,
								$setting,
								array(
									'settings'		=> $setting,
...
function tokopress_customize_controls_enqueue_scripts() {
	$assets_uri = apply_filters( 'tokopress_customize_assets_uri', get_template_directory_uri() . '/assets' );
	...
	wp_enqueue_script( 'tokopress-customize', $assets_uri . '/js/customize.min.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-slider', 'jquery-ui-button', 'jquery-ui-sortable', 'wp-color-picker', 'select2' ), TOKOPRESS_CUSTOMIZER_VERSION, true );
	wp_enqueue_style( 'tokopress-customize', $assets_uri . '/css/customize.min.css', array( 'wp-color-picker', 'select2' ), TOKOPRESS_CUSTOMIZER_VERSION );
}

function tokopress_customize_output() {
	$style = '';
	$fonts = array();
	$fields = apply_filters( 'tokopress_customize_controls', array() );
	if ( ! empty( $fields ) ) {
...
add_filter( 'tokopress_style', 'tokopress_style_customize_css' );
function tokopress_style_customize_css( $style ) {
	$css = '';
	if ( ! is_customize_preview() ) {
		if ( defined( 'TOKOPRESS_CUSTOMIZER_DEBUG' ) && TOKOPRESS_CUSTOMIZER_DEBUG ) {
			$style = $style.' /* Customizer Debug is ON */ ';
			$output = tokopress_customize_output();
			$css = $output['style'];
		}
		else {
			$saved =  get_theme_mod( 'tokopress_customize_saved' );
			if ( $saved ) {
...
add_action( 'tokopress_custom_styles', 'tokopress_style_customize_css_custom' );
function tokopress_style_customize_css_custom() {
	echo apply_filters( 'tokopress_style', '' );
}
...
function tokopress_customize_preview_init_script_css() {
	$fields = apply_filters( 'tokopress_customize_controls', array() );
	if ( ! empty( $fields ) ) {
		echo '<script type="text/javascript" id="tokopress-customize-preview"> ( function( $ ) { head = $(\'head\'); '."\n";
		foreach ( $fields as $field ) {
			$defaults = array(
				'setting_type'			=> tokopress_customize_setting_type(),
				'setting'				=> '',
...
function tokopress_customize_sanitize_number( $val ) {
	return is_numeric( $val ) ? $val : '';
}

function tokopress_customize_sanitize_url( $url ) {
	return esc_url_raw( $url );
}

function tokopress_customize_sanitize_email( $email, $setting ) {
	$email = sanitize_email( $email );
	return ( ! null( $email ) ? $email : $setting->default );
}

function tokopress_customize_sanitize_checkbox( $checked ) {
	return ( ( isset( $checked ) && true == $checked ) ? true : false );
}

function tokopress_customize_sanitize_select( $input, $setting ) {
...
function tokopress_customize_sanitize_select_multiple( $input, $setting ) {
	$input_keys = $input;
	$choices = $setting->manager->get_control( $setting->id )->choices;
...
function tokopress_customize_sanitize_select_remote( $input, $setting ) {
	return esc_attr( $input );
}

function tokopress_customize_sanitize_image( $image, $setting ) {
    $mimes = array(
...
function tokopress_customize_sanitize_color( $color, $setting ) {
...
function tokopress_customize_sanitize_pages( $page_id, $setting ) {
...
function tokopress_customize_sanitize_unfiltered( $html ) {
...
add_action( 'customize_preview_init', 'tokopress_customize_preview_loading_style_init' );
function tokopress_customize_preview_loading_style_init() {
	global $wp_customize;
	remove_action( 'wp_head', array( $wp_customize, 'customize_preview_loading_style' ) );
	add_action( 'wp_head', 'tokopress_customize_preview_loading_style_head', 100 );
	add_action( 'wp_footer', 'tokopress_customize_preview_loading_style_footer' );
}

function tokopress_customize_preview_loading_style_head() {
	echo '<style>';
	echo '/* Spinkit https://github.com/tobiasahlin/SpinKit MIT License */ ';
	echo 'body.wp-customizer-unloading{opacity:1;cursor:progress!important;-webkit-transition:none;transition:none}body.wp-customizer-unloading *{pointer-events:none!important}body.wp-customizer-unloading .tokopress-customize-loading{display:block!important}.tokopress-customize-loading{display:none;position:fixed;z-index:999999;top:0;left:0;width:100%;height:100%;background:#fff;background:rgba(255,255,255,.8)}.spinkit-wave{display:block;position:relative;top:50%;left:50%;width:50px;height:40px;margin:-25px 0 0 -25px;font-size:10px;text-align:center}.spinkit-wave .spinkit-rect{display:block;float:left;width:6px;height:50px;margin:0 2px;background-color:#e91e63;-webkit-animation:spinkit-wave-stretch-delay 1.2s infinite ease-in-out;animation:spinkit-wave-stretch-delay 1.2s infinite ease-in-out}.spinkit-wave .spinkit-rect1{-webkit-animation-delay:-1.2s;animation-delay:-1.2s}.spinkit-wave .spinkit-rect2{-webkit-animation-delay:-1.1s;animation-delay:-1.1s}.spinkit-wave .spinkit-rect3{-webkit-animation-delay:-1s;animation-delay:-1s}.spinkit-wave .spinkit-rect4{-webkit-animation-delay:-.9s;animation-delay:-.9s}.spinkit-wave .spinkit-rect5{-webkit-animation-delay:-.8s;animation-delay:-.8s}@-webkit-keyframes spinkit-wave-stretch-delay{0%,100%,40%{-webkit-transform:scaleY(.5);transform:scaleY(.5)}20%{-webkit-transform:scaleY(1);transform:scaleY(1)}}@keyframes spinkit-wave-stretch-delay{0%,100%,40%{-webkit-transform:scaleY(.5);transform:scaleY(.5)}20%{-webkit-transform:scaleY(1);transform:scaleY(1)}}';
	echo '</style>';
}

function tokopress_customize_preview_loading_style_footer() {
	echo '<div class="tokopress-customize-loading">';
