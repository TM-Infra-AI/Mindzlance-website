...
/* DoubleTabToGo, By Osvaldas Valutis, www.osvaldas.info Available for use under the MIT License */
...
(function($){
    ...
    $(document).ready(function(){
        ...
        /* Sticky header */
        $(".sticky-header-yes #header").sticky({topSpacing:0});

        /* $( ".woocommerce #content div.product .summary p.price, .woocommerce #content div.product .summary span.price, .woocommerce div.product .summary p.price, .woocommerce div.product .summary span.price, .woocommerce-page #content div.product .summary p.price, .woocommerce-page #content div.product .summary span.price, .woocommerce-page div.product .summary p.price, .woocommerce-page div.product .summary span.price, .woocommerce #content div.product .summary form.cart, .woocommerce div.product .summary form.cart, .woocommerce-page #content div.product .summary form.cart, .woocommerce-page div.product .summary form.cart" ).clone().appendTo( ".woocommerce-page div.product .wc-main-content-right .product-calltoaction" ); */

        if ( $('body').hasClass('rtl') ) {
	        $( "#header-menu" ).clone().appendTo( ".sb-slidebar.sb-right" ).removeClass('header-menu').addClass('menu-slidebar menu-slidebar-header').removeAttr('id');
	        $( "#site-navigation-menu" ).clone().appendTo( ".sb-slidebar.sb-right" ).removeClass('site-navigation-menu').addClass('menu-slidebar menu-slidebar-sitenav').removeAttr('id');
	        $( ".sb-slidebar.sb-right ul.sub-menu" ).before( "<i class='sub-menu-arrow fa fa-angle-left'></a>" );
	        $( ".sb-slidebar.sb-right .sub-menu-arrow" ).click(function() {
	        ...
	        $( "#header-menu" ).clone().appendTo( ".sb-slidebar.sb-left" ).removeClass('header-menu').addClass('menu-slidebar menu-slidebar-header').removeAttr('id');
	        $( "#site-navigation-menu" ).clone().appendTo( ".sb-slidebar.sb-left" ).removeClass('site-navigation-menu').addClass('menu-slidebar menu-slidebar-sitenav').removeAttr('id');
	        $( ".sb-slidebar.sb-left ul.sub-menu" ).before( "<i class='sub-menu-arrow fa fa-angle-right'></a>" );
	        $( ".sb-slidebar.sb-left .sub-menu-arrow" ).click(function() {
		...
		$(document).on({
		    mouseenter: function () {
	            $('#lang-menu-chooser').show();
		    },
		    mouseleave: function () {
	            $('#lang-menu-chooser').hide();
		    }
		}, ".quicknav-lang");
		...
		$( '.header-menu li.menu-item:has(ul), .site-navigation-menu li.menu-item:has(ul)' ).doubleTapToGo();

        $(".quicknav-search .quicknav-icon, .quicknav-close").on("click", function(){
            if($('.site-header .header-right-search:visible').length) {
                $('.site-header .header-right-search').hide();
                $('.site-header .header-right').fadeIn();
            }
            else {
                $('.site-header .header-right').hide();
                $('.site-header .header-right-search').fadeIn();
            }
        });

        // Register Vendor Checked
        $("#reg_billing_vendor_wrap").hide();
        $("#reg_billing_for_vendor").click(function() {
            if($(this).is(":checked")) {
                $("#reg_billing_vendor_wrap").show(300);
            } else {
                $("#reg_billing_vendor_wrap").hide(200);
            }
        });

        $('.post_tag-cloud a').removeAttr('style');

        $('.widget_tag_cloud a').removeAttr('style');

        $('.widget_product_tag_cloud .tagcloud a').removeAttr('style');
        ...
        if(jQuery.isFunction($().owlCarousel)) {
	        ...
	        if ( $('body').hasClass('rtl') ) {
	            ...
	            $('.format-gallery .entry-content .framebox ul').owlCarousel({
					rtl:true,
	                items : 1,
	                loop: true,
	                nav : true,
		...
		$('.view-store-link').insertAfter('.woocommerce div.product .woocommerce-tabs ul.tabs');

        // set footer widget height
        $("#footer .footer-widgets").css("min-height", '').css("min-height", $("#footer").height());
        ...
        /* Modified JRMenuMore https://github.com/SodhanaLibrary/jRMenuMore */
        ...
        $( "#site-navigation-menu").jRMenuMore();
        ...
        $( 'ul.dokan-seller-wrap li.dokan-single-seller .dokan-store-caption' ).wrapInner( "<div class='dokan-store-caption-inner'></div>" );

        if ( $(window).width() < 992 ) {
            $( '.woocommerce .product-section-description' ).appendTo( $( '.woocommerce .product-section-bottom-left' ) );
            $( '.woocommerce .product-section-detail' ).appendTo( $( '.woocommerce .product-section-bottom-right' ) );
        } 
        else {
            $( '.woocommerce .product-section-description' ).appendTo( $( '.woocommerce .product-section-top-left' ) );
            $( '.woocommerce .product-section-detail' ).appendTo( $( '.woocommerce .product-section-top-right' ) );
        }
        $(window).resize(function() {
            if ( $(window).width() < 992 ) {
                $( '.woocommerce .product-section-description' ).appendTo( $( '.woocommerce .product-section-bottom-left' ) );
		...
		$('#support-submit-btn').dblclick(function(e){
			console.log('agus');
			e.preventDefault();
		});
    ...
    $("aside#sidebar .menu-item-has-children>a").removeAttr("href");
$( "aside#sidebar .menu-item-has-children" ).click(function() {
