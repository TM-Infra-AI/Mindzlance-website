        ...
        <div class="profile-info">
            <ul class="store-info dokan-store-info">
                ...
                <?php if ( isset( $store_info['address'] ) && !empty( $store_info['address'] ) ) { ?>
                    <?php if ( $address = dokan_get_seller_address( $store_user->ID ) ) { ?>
                        <li class="dokan-store-address"><i class="fa fa-map-marker"></i>
                            <?php echo str_replace( array( '<br>','<br/>' ), ' ', $address ); ?>
                        </li>
                    <?php } ?>
                <?php } ?>
            ...
            <?php if ( isset( $store_info['dokan_store_time_enabled'] ) && $store_info['dokan_store_time_enabled']=='yes') {
                    echo "<table class='dokan_store_info_custom' style='margin:0 auto; color:#FFF;'>";
                    foreach ($store_info['dokan_store_time'] as $key => $value) {
                    ?><tbody><tr><?php
                       echo "<td>Day: ".$key."</td><td>" ;
