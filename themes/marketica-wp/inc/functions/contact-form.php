...
// "sales@mindzlance.com,romil@marketingmindz.com"
	...
	$notification = false;
	$email_sent = false;
	if ( ( count( $_POST ) > 4 ) && isset( $_POST['submitted'] ) ) {
		...
		if ( isset ( $_POST['g-recaptcha-response'] ) && $_POST['g-recaptcha-response'] != '' )
			$message_re = "Your Message: ".$_POST['g-recaptcha-response'] . "\n\r\n\r";
		else
			$error_messages['g-recaptcha-response'] = __( 'Please enter your recaptcha', 'tokopress' );
	...
	if ( $email == '' ) {
		...
		if( array_key_exists( 'contact-name', $error_messages ) ) {
			$html .= '<span class="contact-error alert alert-danger">' . $error_messages['contact-name'] . '</span>' . "\n";
		}
		$html .= '</p>' . "\n";
		...
		if( array_key_exists( 'contact-email', $error_messages ) ) {
			$html .= '<span class="contact-error alert alert-danger">' . $error_messages['contact-email'] . '</span>' . "\n";
		}
		$html .= '</p>' . "\n";
		$html .= '</div>' . "\n";
		...
		if( array_key_exists( 'contact-message', $error_messages ) ) {
			$html .= '<span class="contact-error alert alert-danger">' . $error_messages['contact-message'] . '</span>' . "\n";
		}
		$html .= '</p>' . "\n";
		$html .= '</div>' . "\n";
		...
		if ( $question && $answer ) {
			...
			if( array_key_exists( 'contact-quiz', $error_messages ) ) {
				$html .= '<span class="contact-error alert alert-danger">' . $error_messages['contact-quiz'] . '</span>' . "\n";
			}
			$html .= '</p>' . "\n";
		}
		...
		$html .= '<div class="block-column">' . "\n";
		$html .= '<p class="inline"><div class="g-recaptcha" data-sitekey="6LepTpoUAAAAALHupLyTM3Cn5kFEY20sOdQlB36M"></div></p>' . "\n";
		if( array_key_exists( 'g-recaptcha-response', $error_messages ) ) {
			$html .= '<span class="contact-error alert alert-danger">' . $error_messages['g-recaptcha-response'] . '</span>' . "\n";
		}
		$html .= '</div>' . "\n";
